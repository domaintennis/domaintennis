...
// Add Zen Tabs styles
if (theme_get_setting('cruise_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'cruise') .'/css/tabs.css');
}

function cruise_preprocess_html(&$vars) {
  global $user;

  //Add role name classes (to allow css based show for admin/hidden from user)
  foreach ($user->roles as $role){
    $vars['classes_array'][] = 'role-' . cruise_id_safe($role);
  }
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = cruise_id_safe('page-'. $path);
    $vars['classes_array'][] = cruise_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('cruise_ie_enabled')) {
    $cruise_ie_enabled_versions = theme_get_setting('cruise_ie_enabled_versions');
    if (in_array('ie8', $cruise_ie_enabled_versions, TRUE)) {
...
function cruise_preprocess_page(&$vars, $hook) {
...
function cruise_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from cruise_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function cruise_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function cruise_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('cruise_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('cruise_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('cruise_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('cruise_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('cruise_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function cruise_id_safe($string) {
 ...
 *
 */
function cruise_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = cruise_id_safe($element['#title']);
 ...
 */
function cruise_preprocess_menu_local_task(&$variables) {
 ...
 */
function cruise_menu_local_tasks(&$variables) {
  $output = '';
...
function cruise_js_alter(&$js) {
  if (request_uri() == '*/edit' || request_uri() =='node/add/page') {
    $js_path = drupal_get_path('theme', 'cruise') . "/js/cdu.js";
    if (isset($js[$js_path])) {
      unset($js[$js_path]);
    }
  }
